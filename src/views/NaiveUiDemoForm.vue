    ...
    <div>
        ...
        >
          ...
          <n-form-item label="日期時間" path="datetimeValue">
            <n-date-picker v-model:value="model.datetimeValue" type="datetime" placeholder="請選擇日期時間"/>
          </n-form-item>
          <n-form-item label="日期區間" path="daterangeValue">
            <Daterange v-model:value="model.daterangeValue" @returnDaterange="setDaterange($event)" />
          </n-form-item>
          ...
          <n-form-item label="單選選項按鈕" path="radioGroupValue">
            <n-radio-group v-model:value="model.radioButtonGroupValue" name="radioButtongroup2">
              <n-radio-button value="RadioButton1">
                選項按鈕 1
              </n-radio-button>
              <n-radio-button value="RadioButton2">
                選項按鈕 2
              </n-radio-button>
              <n-radio-button value="RadioButton3">
                選項按鈕 3
              </n-radio-button>
            </n-radio-group>
          </n-form-item>
          <n-form-item label="數量" path="inputNumberValue">
            <n-input-number v-model:value="model.inputNumberValue" placeholder="數量" :style="{maxWidth: '100px'}"/>
          </n-form-item>
          ...
          <div style="display: flex; justify-content: center">
            <n-button round type="default" @click="reset" style="margin-right: 10px;">
              重設
            </n-button>
            <n-button round type="primary" @click="handleValidateButtonClick">
              送出
...
export default defineComponent({
  ...
  setup() {
    ...
    return {
      ...
      size: ref("medium"),
      setDaterange(value){
          this.model.daterangeValue = value
          console.log(this.model.daterangeValue)
      },
      reset() {
      ...
      model,
      generalOptions: ["實體商店", "網路商店", "卡機商店"].map(
        (v) => ({
          label: v,
          value: v
        })
      ),
      shopAttrOptions: ["實體商店", "網路商店", "卡機商店"].map(
        (v) => ({
          label: v,
          value: v
        })
      ),
      productOptions: ["食品","服飾", "鞋包配件", "居家","3C家電","其它"].map(
        (v) => ({
          label: v,
          value: v
        })
